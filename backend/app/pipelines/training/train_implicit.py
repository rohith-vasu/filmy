...
def get_current_production_metrics(model_name="filmy_implicit_model"):
    client = MlflowClient()
    versions = client.search_model_versions(f"name='{model_name}' and tags.stage='production'")
    if not versions:
...
def train():
    ...
    # Build dataset_map using full catalog (pass db)
    dataset_map = build_dataset_map(feedbacks, db)
    user_map = dataset_map["user_map"]
    item_map = dataset_map["item_map"]

    # Build interactions sparse matrix
    interactions = build_interaction_matrix(feedbacks, user_map, item_map, alpha=ALPHA)
    logger.info(f"Interaction matrix shape: {interactions.shape}, nnz={interactions.nnz}")
    ...
    with mlflow.start_run(run_name="implicit_als") as run:
        ...
        artifact_dir = save_artifacts_to_tempdir(model, dataset_map, interactions, ARTIFACT_TEMP_DIR)
        ...
        if is_better(p10, r10, prod_p10, prod_r10):
            ...
            client.set_model_version_tag(name="filmy_implicit_model", version=latest.version, key="stage", value="production")
            if prod_version:
                client.delete_model_version_tag(name="filmy_implicit_model", version=prod_version.version, key="stage")
            logger.info(f"Promoted model v{latest.version}")
        else:
            logger.info("New model not better; skipping promotion")
